...
type HostGame = { code: string; phase: Phase; roundIndex: number; revision: number; players: Player[]; answered: string[]; round?: Round; voteSplit?: { key: string; choice: string; names: string[] }[] };
...
export default function Host() {
  ...
  return <main className="game-shell host-shell">
    ...
    {game?.phase === "lobby" && <section className="host-lobby panel-enter">
      ...
      {error && <p className="error">{error}</p>}<button className="primary" disabled={game.players.length === 0 || busy} onClick={() => act("start")}>{busy ? "STARTING…" : "START THE CHAOS"} <span>→</span></button>
    </section>}
    ...
    {game?.phase === "question" && round && <section className="question panel-enter">
      ...
      <button className="primary" disabled={game.answered.length !== game.players.length || busy} onClick={() => act("reveal")}>{busy ? "REVEALING…" : "REVEAL THE TRUTH"} <span>→</span></button>
    </section>}
